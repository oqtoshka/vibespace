...
/**
 * Backend cache metadata for one provider model lookup: `updatedAt` is when
 * the catalog was last fetched from the provider, `expiresAt` the cache expiry,
 * and `source` whether the response came from memory, disk, or a fresh fetch.
 */
 ...
 * the selectable catalog plus the cache metadata that says how current it is.
 */
 ...
 * next resumed turn. A backend-owned session override, not a claim that the
 * provider has already switched the running session in-place.
 */
...
/**
 * Provider-neutral session model-change state. `supported` says whether the
 * adapter supports the session-scoped resume override at all; `changed` is
 * the persisted flag the resume layer checks; when it is `false`, `model` is
 * `null` and the normal request/default selection applies.
 */
...
export type ProviderPermissionDecision = {
  ...
  /**
   * Permission mode to continue in once an interactive approval (ExitPlanMode)
   * is granted, so the build phase starts with the mode the user picked while
   * reading the plan instead of the runtime's own `default`.
   */
