...
export function sessionAvatarUrl(
  ...
  if (isPrivate || !providerSessionId?.trim()) {
    return null;
  }
 ...
 * overrides.
 *
 * The file lives under `~/.vibespace` because these overrides are an application
...
export function getProviderSessionActiveModelChangesPath(): string {
  return path.join(os.homedir(), '.vibespace', 'provider-session-active-model-changes.json');
}
