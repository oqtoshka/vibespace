...
type ProviderRuntimeServiceDependencies = {
  ...
  /** Resolves to the merged catalog definition (cache metadata stripped). */
  getProviderModels(provider: LLMProvider): Promise<ProviderModelsDefinition>;
};
...
const defaultDependencies: ProviderRuntimeServiceDependencies = {
  ...
  resolveResumeModel: (provider, sessionId, requestedModel, options) =>
    providerModelsService.resolveResumeModel(provider, sessionId, requestedModel, options),
  getProviderModels: async (provider) => (await providerModelsService.getProviderModels(provider)).models,
};
...
export function createProviderRuntimeService(
  ...
  const createRuntimeContext = (
    ...
    resolveResumeModel: (sessionId, requestedModel, options) =>
      dependencies.resolveResumeModel(provider.id, sessionId, requestedModel, options),
