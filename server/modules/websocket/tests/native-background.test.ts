...
test('only a live Claude session reports its own running set', () => {
  ...
  const a = nativeBackgroundSnapshot('app-a', { provider: 'claude', provider_session_id: 'prov-a' }, rt, 7);
  ...
  const b = nativeBackgroundSnapshot('app-b', { provider: 'claude', provider_session_id: 'prov-b' }, rt, 7);
...
test('a session not in memory is unavailable, never an empty running set', () => {
  const snap = nativeBackgroundSnapshot('app-c', { provider: 'claude', provider_session_id: 'gone' }, runtime({}), 1);
...
test('providers without a runtime inventory stay unknown', () => {
  for (const provider of ['codex', 'cursor', 'opencode', 'gemini']) {
    const snap = nativeBackgroundSnapshot('x', { provider }, runtime({ x: [{ taskId: 't', description: '' }] }), 1);
...
test('falls back to the app id and bounds hostile task fields', () => {
  ...
  const snap = nativeBackgroundSnapshot('app-d', { provider: 'claude', provider_session_id: null },
