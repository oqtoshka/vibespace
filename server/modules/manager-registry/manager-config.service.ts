...
function parseUserMap(env: NodeJS.ProcessEnv, { requirePasswordHash }: { requirePasswordHash: boolean }) {
  ...
  try {
    ...
    throw new Error(`Manager user map is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
...
function resolveJwtSecret(env: NodeJS.ProcessEnv) {
...
/** Legacy manager entrypoints consume the validated static or controller-owned configuration. */
export function loadManagerConfig(env: NodeJS.ProcessEnv, oidc: unknown) {
  const authKind = env.VS_MANAGER_AUTH || 'password';
  if (env.VS_MANAGER_REGISTRY_FILE && authKind !== 'oidc') throw new Error('Dynamic registry requires OIDC');
  const { credentials, links } = env.VS_MANAGER_REGISTRY_FILE
    ? { credentials: new Map(), links: new WorkerRegistry(env.VS_MANAGER_REGISTRY_FILE) }
    : parseUserMap(env, { requirePasswordHash: authKind === 'password' });
