...
const API_URL = process.env.ANTHILL_API_URL ?? 'https://anthill.example.com/api/graphql';
...
const PROJECT_PATH = process.env.ANTHILL_MAC_PROJECT ?? '/Users/me/projects/anthill';
...
function buildPrompt(task: QueuedTask): string {
  return [
    ...
    '   export ANTHILL_API_URL=https://anthill.example.com/api/graphql',
    ...
    `   Stuck on a question only the user can answer: anthill block-task --id ${task.id} --reason "<the question>"`,
    '4. NEVER mark the task DONE — acceptance is the user\'s alone, REVIEW is as far as you go.',
    '5. File discovered follow-up work with anthill create-task under the same desiredState.',
    '',
    'Mission Control (https://mc.example.com) is the board this run appears on. Two things make it legible there, both before you start work:',
