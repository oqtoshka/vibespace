...
// the UI can render `default (Opus 5.5 with 1M context)` instead of a bare
// `default`. The concrete model only lives in the option's human description
// (e.g. "Use the default model (currently Opus 5.5 with 1M context) · …"), so we
...
const executeModelsCommand = async (args, context, modelsService) => {
  ...
  const { models: catalog } = await modelsService.getProviderModels(currentProvider);
...
`;
  ...
  "/cost": async (args, context) => {
    ...
    const { models: catalog } = await providerModelsService.getProviderModels(provider);
  ...
  "/status": async (args, context) => {
    ...
    const { models: statusCatalog } = await providerModelsService.getProviderModels(statusProvider);
