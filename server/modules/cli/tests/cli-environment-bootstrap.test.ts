...
async function runCliBootstrapFixture(isPlatform: boolean): Promise<BootstrapResult> {
  ...
  try {
    ...
    await mkdir(path.join(fixtureServerDirectory, 'shared'), { recursive: true });
    await Promise.all([
      // load-env registers server-only env keys with the agent-env module.
      copyFile(
        path.join(applicationRoot, 'server', 'shared', 'agent-env.ts'),
        path.join(fixtureServerDirectory, 'shared', 'agent-env.ts'),
      ),
      copyFile(
