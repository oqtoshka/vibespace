...
# Auto-start VibeSpace server in background if not already running.
# This script is sourced from ~/.bashrc on sandbox shell open.

if ! pgrep -f "server/index.js" > /dev/null 2>&1; then
  nohup vibespace start --port 3001 > /tmp/vibespace-ui.log 2>&1 &
  disown

  echo ""
  echo "  VibeSpace is starting on port 3001..."
  echo ""
