...
const ORIGIN = 'https://vs.example.com';
...
test('resolves the reported same-origin generated image URL', () => {
  assert.equal(
    resolveGeneratedImageArtifactPath(
      `${ORIGIN}/Users/me/.codex/generated_images/${RELATIVE_IMAGE}`,
      ORIGIN,
    ),
    RELATIVE_IMAGE,
  );
...
test('resolves root-absolute, file, encoded, and Windows generated image paths', () => {
  assert.equal(
    resolveGeneratedImageArtifactPath(`/Users/me/.codex/generated_images/${RELATIVE_IMAGE}`, ORIGIN),
    RELATIVE_IMAGE,
  );
  assert.equal(
    resolveGeneratedImageArtifactPath(`file:///Users/me/.codex/generated_images/${RELATIVE_IMAGE}`, ORIGIN),
    RELATIVE_IMAGE,
  );
  assert.equal(
    resolveGeneratedImageArtifactPath('/Users/me/.codex/generated_images/folder%20name/image.png', ORIGIN),
    'folder name/image.png',
  );
  assert.equal(
    resolveGeneratedImageArtifactPath('C:\\Users\\homelab\\.codex\\generated_images\\run\\image.webp', ORIGIN),
    'run/image.webp',
  );
});
...
test('rejects remote, traversal, unrelated, and active image URLs', () => {
  ...
  assert.equal(resolveGeneratedImageArtifactPath('/Users/me/.codex/generated_images/../auth.png', ORIGIN), null);
  assert.equal(resolveGeneratedImageArtifactPath('/Users/me/.codex/generated_images/run/image.svg', ORIGIN), null);
...
test('resolves provider-neutral project image references', () => {
  const projectRoot = '/Users/me/projects/mission-control';
...
test('rejects remote and out-of-project absolute image references', () => {
  ...
  assert.equal(resolveProjectImagePath('/Users/me/.ssh/secret.png', projectRoot, ORIGIN), null);
