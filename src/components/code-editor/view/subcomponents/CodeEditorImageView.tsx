...
/**
 * Renders an image inline as an editor tab (replaces the old modal viewer).
 *
 * The bytes are fetched through the authenticated blob endpoint
 * (`GET /api/projects/:id/files/content`) and turned into an object URL — an
 * `<img src>` can't carry the app's Bearer token, so we can't point it at the
 * API directly. The object URL is revoked on unmount / when the file changes.
 */
export default function CodeEditorImageView({
  file,
  isSidebar,
  isFullscreen,
  onClose,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setStatus('error');
        setErrorMessage((error as Error)?.message || 'Could not load this image.');
      }
    })();
    ...
    <div className="relative h-full w-full overflow-auto bg-muted">
      {status === 'error' ? (
        <div className="flex h-full w-full items-center justify-center p-8 text-center text-sm text-muted-foreground">
          <div>
            <p className="font-medium text-foreground">Couldn’t display this image</p>
