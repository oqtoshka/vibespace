...
const NAV_TABS: Array<{ id: AppTab; labelKey: string; keywords: string }> = [
  { id: 'chat', labelKey: 'commandPalette.goTo.chat', keywords: 'chat messages conversation session' },
  { id: 'files', labelKey: 'commandPalette.goTo.files', keywords: 'files file tree explorer' },
  { id: 'shell', labelKey: 'commandPalette.goTo.shell', keywords: 'shell terminal console' },
  { id: 'git', labelKey: 'commandPalette.goTo.git', keywords: 'git diff branches' },
  { id: 'tasks', labelKey: 'commandPalette.goTo.tasks', keywords: 'tasks taskmaster' },
];
...
export default function CommandPalette({
  ...
  const pageLabel = page ? t(`commandPalette.pages.${page}`) : '';
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-xl overflow-hidden p-0">
        ...
        <Command label={t('commandPalette.title')} onKeyDown={handleKeyDown}>
          ...
          <CommandInput
            placeholder={page
              ? t('commandPalette.searchPage', { page: pageLabel.toLocaleLowerCase() })
              : t('commandPalette.searchAll')}
            value={search}
            ...
            {showActions && projectId && (
              <CommandGroup heading={t('commandPalette.groups.git')}>
                ...
                >
                  ...
                  <span className="flex-1">{t('commandPalette.gitActions.fetch')}</span>
                </CommandItem>
                <CommandItem
                ...
                >
                  ...
                  <span className="flex-1">{t('commandPalette.gitActions.pull')}</span>
                </CommandItem>
                <CommandItem
                ...
                >
                  ...
                  <span className="flex-1">{t('commandPalette.gitActions.push')}</span>
                </CommandItem>
              </CommandGroup>
            )}
                ...
                {SETTINGS_MAIN_TABS.map(({ id, label, keywords, icon: Icon }) => (
                  ...
                  >
                    ...
                    <span className="flex-1">
                      {t('commandPalette.settingsItem', {
                        label: t(`settings:mainTabs.${id === 'api' ? 'apiTokens' : id}`, { defaultValue: label }),
                      })}
                    </span>
                  </CommandItem>
                ))}
            ...
            {showSessions && projectId && sessionsShown.length > 0 && (
              <CommandGroup heading={t('commandPalette.groups.sessions')}>
                ...
                {!page && sessionRows.length > browseLimit && (
                  <BrowseAllItem label={t('commandPalette.browseAll', { section: t('commandPalette.pages.sessions').toLocaleLowerCase(), count: sessionRows.length })} onSelect={() => pushPage('sessions')} />
                )}
              </CommandGroup>
            )}
            ...
            {showFiles && projectId && filesShown.length > 0 && (
              <CommandGroup heading={t('commandPalette.groups.files')}>
                ...
                {!page && files.length > browseLimit && (
                  <BrowseAllItem label={t('commandPalette.browseAll', { section: t('commandPalette.pages.files').toLocaleLowerCase(), count: files.length })} onSelect={() => pushPage('files')} />
                )}
              </CommandGroup>
            )}
            ...
            {showCommits && projectId && commitsShown.length > 0 && (
              <CommandGroup heading={t('commandPalette.groups.commits')}>
                ...
                {!page && commits.length > browseLimit && (
                  <BrowseAllItem label={t('commandPalette.browseAll', { section: t('commandPalette.pages.commits').toLocaleLowerCase(), count: commits.length })} onSelect={() => pushPage('commits')} />
                )}
              </CommandGroup>
            )}
              ...
              <CommandGroup heading={t('commandPalette.groups.branches')}>
                {branchesShown.map((b) => (
                  ...
                  >
                    ...
                    <span className="flex-1 truncate">{t('commandPalette.switchToBranch', { branch: b.name })}</span>
                  </CommandItem>
                ))}
                {!page && branches.length > browseLimit && (
                  <BrowseAllItem label={t('commandPalette.browseAll', { section: t('commandPalette.pages.branches').toLocaleLowerCase(), count: branches.length })} onSelect={() => pushPage('branches')} />
                )}
              </CommandGroup>
            )}
          </CommandList>
