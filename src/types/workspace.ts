...
/**
 * Panels are singleton views (Files/Git/Tasks/Preview/plugins) selected from
 * the pill bar; they are not closable tabs. Their ids share one selection
 * space with workspace tab ids (`tab_*`), discriminated by `isPanelId`.
 */
export type WorkspacePanel = 'files' | 'git' | 'tasks' | 'preview' | `plugin:${string}`;
...
const PANEL_IDS: ReadonlySet<string> = new Set(['files', 'git', 'tasks', 'preview']);
