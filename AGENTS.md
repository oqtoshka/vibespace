...
```bash
# from this repo, on the Mac that hosts cloud.example.com
npm run build                                   # regenerates dist/ + dist-server/
launchctl kickstart -k "gui/$(id -u)/com.example.vibespace"
```

> **Note on the service id:** the launchd rename landed (2026-06-11) — the
> LaunchAgent is now `com.example.vibespace` (+ watchdog
> `com.example.vibespace-watchdog`). The legacy `com.example.cloudcli*` labels are
...
- **LaunchAgent** `com.example.vibespace` (+ watchdog `com.example.vibespace-watchdog`)
...
- nginx (on spider/soyuz) reverse-proxies cloud.example.com → the WireGuard IP:port.
...
- The repo `.npmrc` points scoped packages at the **private the private registry**
  (`@*:registry=https://npm.internal.invalid/`); auth token lives in `~/.npmrc`.
...
- **The wrapper must stay executable.** The LaunchAgent runs
  `exec /Users/me/.local/bin/vibespace-wrapper.sh` *directly* (not
  ...
  forever — nginx then returns **502** on cloud.example.com. This bit gets dropped
  ...
  `chmod +x /Users/me/.local/bin/vibespace-wrapper.sh && launchctl kickstart -k "gui/$(id -u)/com.example.vibespace"`
